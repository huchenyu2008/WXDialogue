...
#ifdef _MSC_VER
#define WXDL_ERR_LOG(state, call, pid, text, ...) \
    {\
        static char _wxdl_log_info_buff[128];\
        snprintf(_wxdl_log_info_buff, 1024, text, __VA_ARGS__);\
        wxdl_log_call_error(state, call, _wxdl_log_info_buff, pid);\
    }
...
WXDIALOGUE_API void wxdl_log_call_error(WXDLstate* state, WXDLcall* call, const WXDLchar* text, WXDLu32 pid);
